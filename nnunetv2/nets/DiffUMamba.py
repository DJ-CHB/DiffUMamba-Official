...
class UNetResEncoder(nn.Module):
    ...
    def add_uniform_noise(self, data, low = -2, high = 2):
        noise = torch.empty_like(data, device=data.device).uniform_(low, high)  
        return data + noise

    def add_speckle_noise(self, data, scale=0.5):
        noise = torch.empty_like(data, device=data.device).normal_(0, scale)
        return data + data * noise

    def add_salt_and_pepper_noise(self, data, prob=0.002):
        mask = torch.rand_like(data, device=data.device)
        data[mask < prob / 2] = data.min()  # Salt
        data[mask > 1 - prob / 2] = data.max()  # Pepper
        return data

    def add_periodic_noise(self, data, frequency=10, amplitude=3.5):
        x = torch.arange(data.shape[-1], device=data.device).float()
        sine_wave = amplitude * torch.sin(2 * torch.pi * frequency * x / data.shape[-1])
...
class DiffUMamba(nn.Module):
    def __init__(self,
        ...
        self.mamba_m_1= MambaLayer(dim = features_per_stage[-1])
        self.mamba_m_2= MambaLayer(dim = features_per_stage[-1])
...
def get_diffumamba_3d_from_plans(
    ...
    segmentation_network_class_name = 'DiffUMamba'
    network_class = DiffUMamba
    kwargs = {
        'DiffUMamba': {
            'conv_bias': True,
